...
class OllamaDiscovery:
        ...
        for manifest in manifest_root.rglob("*"):
            ...
            try:
                ...
                if info.available:
                    ...
                    LOG.warning(
                        "Skipping unavailable GGUF %s: %s",
                        info.name,
                        info.error,
                    )
            except (OSError, json.JSONDecodeError, ValueError) as exc:
                LOG.warning(
                    "Skipping invalid Ollama manifest %s: %s",
                    manifest,
                    exc,
                )
    ...
    @staticmethod
    def validate_gguf(path: Path, expected_size: int = 0) -> str | None:
    ...
    def _parse_manifest(self, path: Path, manifest_root: Path) -> ModelInfo:
        ...
        for layer in layers:
            ...
            if candidate.exists() and ("model" in media_type or blob is None):
                ...
                validation_error = self._validate_gguf(
                    candidate,
                    expected_size,
                )
