...
def _decode_record(payload: SerializedAnalysisRecord) -> AnalysisRecord:
    """Reconstruct one cached analysis record from its JSON representation."""
...
class AnalysisPageCache:
    ...
    def enabled(self) -> bool:
        """Return whether disk paging is enabled."""
        return self.max_bytes > 0

    @property
    def record_count(self) -> int:
        """Return the total number of paged and pending records."""
        return (
                sum(
                    count
                    for _path, count, _size in self._pages
                )
                + len(self._pending)
        )
    ...
    def disk_bytes(self) -> int:
        """Return the number of bytes occupied by retained cache pages."""
        return sum(
            size
            for _path, _count, size in self._pages
        )

    @property
    def directory(self) -> Path | None:
        """Return the current process cache directory, when allocated."""
        return self._directory

    def set_limit(self, max_bytes: int) -> None:
        """Change the cache limit and immediately enforce the new ceiling."""
        self.max_bytes = max(
            0,
            int(max_bytes),
        )
    ...
    def iter_records(self) -> Iterator[AnalysisRecord]:
        ...
        for path, _count, _size in self._pages:
            with gzip.open(
                ...
                for line in handle:
                    if line.strip():
                        payload: SerializedAnalysisRecord = json.loads(line)
    ...
    def status(
        ...
        """Return current cache usage and retention state."""
        return {
    ...
    def _flush(self) -> None:
        """Write the pending records into one compressed chronological page."""
        ...
        page = (
                directory
                / f"page-{self._next_page:08d}.jsonl.gz"
        )

        temporary = page.with_suffix(
            page.suffix + ".tmp"
        )
    ...
    def _enforce_limit(self) -> None:
        """Discard the oldest pages until the cache fits within its limit."""
        if not self.enabled:
            return
    ...
    def _pid_is_running(pid: int) -> bool:
        """Return whether a process ID still represents a live process."""
        if pid == os.getpid():
            return True

        if os.name == "nt":
            return kernel32.process_exists(pid)
