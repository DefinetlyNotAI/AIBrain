...
class GpuLaunchTests(unittest.TestCase):
    ...
    def test_supervisor_terminates_the_child_when_ctrl_c_interrupts_waiting(
        ...
        self.assertEqual(
            gpu.supervise_gpu_launch(["python.exe", "cli/diagnostic.py"]), 130
        )
    ...
    def test_venv_gpu_preferences_include_the_base_python_process_image(self) -> None:
        with (
            patch.object(
                gpu.sys, "executable", str(Path("env/Scripts/python.exe").resolve())
            ),
            patch.object(
                gpu.sys, "_base_executable", str(Path("base/python.exe").resolve())
            ),
    ...
    def test_every_desktop_launcher_returns_supervised_exit_before_creating_qt(
        ...
        for launcher in (main, diagnostic, analysis):
            with (
                ...
                patch.object(
                    launcher, "configure_cli_logging", return_value=("log", "crash")
                ),
    ...
    def test_gpu_preference_is_saved_before_child_launch_and_preserves_arguments(
        ...
        for compiled, expected in (
            ...
            with (
                ...
                patch.object(
                    gpu, "should_prefer_high_performance_gpu", return_value=True
                ),
                patch.object(
                    gpu, "set_windows_gpu_preference", return_value=True
                ) as save,
    ...
    def test_system_default_and_supervised_children_do_not_relaunch(self) -> None:
        for prefer, supervised in ((False, False), (True, True)):
            ...
            with (
                ...
                patch.object(
                    gpu, "should_prefer_high_performance_gpu", return_value=prefer
                ),
