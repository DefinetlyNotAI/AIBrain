...
The display-decay, edge-intensity, and channel-aggregation hot paths live in `src/native/c/connectome_kernels.c`. They
...
The DLL accelerates display-signal bookkeeping. It does not implement the Qt window, OpenGL renderer, or language-model
inference.
...
so the connectome decay, edge, and channel aggregation loops remain the focused native surface.
