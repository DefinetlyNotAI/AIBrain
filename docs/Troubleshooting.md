...
To repair dependencies, run `py installer.py` again. The installer repairs the managed environment rather than the system Python.
...
```powershell
py scripts\build_native.py --compiler "C:\path\to\compiler.exe"
```
