...
class ApplicationTarget:
    """One standalone AIBrain application build target."""
...
def _is_option_echo(line: str) -> bool:
    ...
    prefix, separator, message = line.lstrip().partition(
        "Nuitka-Options:"
    )
...
def _is_build_noise(line: str) -> bool:
    ...
    if re.match(
            r"Nuitka[^:]*:\s*(?:WARNING|ERROR|FATAL):",
            stripped,
    ):
        return False
...
def _write_completed_build_line(
        output_box: BuildLineOutput,
        line: str,
) -> None:
...
def _render_output_text(
    ...
    if (
            visible_partial
            and not _is_build_noise(visible_partial)
    ):
        if re.search(
                r"\d+(?:\.\d+)?%.*?\b\d+/\d+",
                visible_partial,
...
def _command_activity(
    ...
    """Name the running program, module, or script without exposing arguments."""
    executable = Path(
        command_line[0]
    ).name

    name = executable.lower().removesuffix(
        ".exe"
    )
...
def _report_build_heartbeat(
    ...
    interval = (
        ...
        else BUILD_LOG_HEARTBEAT_SECONDS
    )
...
def _monitor_build_stall(
    ...
    if (
            now - last_child_output < BUILD_STALL_SECONDS
            or now < state.next_check
    ):
        return None, state
    ...
    message = (
        f"No new output from {activity} "
        f"for {now - last_child_output:.0f}s. "
        "The process is still alive. "
        f"{explanation}"
        "Monitoring will continue. "
        "Press Ctrl+C once to stop the process tree."
    )
...
def _create_pipe() -> tuple[Handle, Handle]:
    ...
    security.nLength = sizeof(
        SecurityAttributes
    )
    security.lpSecurityDescriptor = None
    security.bInheritHandle = False

    return kernel32.create_pipe(
        security
    )
...
def _create_conpty_process(
    ...
    attribute_buffer: NativeArray[CChar] | None = None
    ...
    try:
        ...
        startup_info.StartupInfo.cb = sizeof(
            StartupInfoExW
        )
        ...
        mutable_command = unicode_buffer(
            command_text
        )
        ...
        process_information.hThread = Handle()
...
def _conpty_reader(
    ...
    """Read ConPTY output until its native pipe closes."""
    try:
        while True:
...
def _run_with_conpty(
    ...
    process = _create_conpty_process(
        command_line
    )
...
def run(
    ...
    _run_with_file_tailer(
        command_line
    )


def runtime_dlls() -> list[Path]:
    """Locate required Windows runtime DLLs."""
    system32 = (
            Path(
                os.environ.get(
                    "SYSTEMROOT",
...
def stage_numpy_runtime(
    ...
    info(
        "Staging the required NumPy runtime files"
    )

    for source in NUMPY_PACKAGE_ROOT.glob(
            "*.py"
    ):
    ...
    for name in NUMPY_RUNTIME_SUBDIRECTORIES:
        ...
        if not source.is_dir():
            raise RuntimeError(
                f"Managed NumPy runtime submodule is missing: {name}"
            )
    ...
    info(
        "NumPy runtime staging complete"
    )
...
def nuitka_command(
    ...
    if numpy_runtime is not None:
        package_root, dll_root = (
            numpy_runtime
        )

        command_line.insert(
            -1,
            f"--include-raw-dir={package_root}=numpy",
        )

        command_line.insert(
            -1,
            f"--include-raw-dir={dll_root}=numpy.libs",
        )
...
def _produced_distribution(
    ...
    """Return the single standalone distribution produced by Nuitka."""
    candidates = list(
...
def _verify_application(
    ...
    """Verify one built standalone application."""
    executable = (
            application
            / target.executable
    )
    ...
    if not native_library.is_file():
        raise RuntimeError(
            f"{target.executable} is missing the native connectome DLL"
        )
    ...
    for runtime_name in (
        ...
        if not (
            ...
            raise RuntimeError(
                f"{target.executable} is missing required runtime "
                f"{runtime_name}"
            )
...
def merge_application_distributions(
    ...
    merged = (
            release
            / MERGED_APPLICATION_DIRECTORY
    )

    staging = (
            release
            / f"_{MERGED_APPLICATION_DIRECTORY}.merge"
    )
    ...
    info(
        f"Creating merged application folder: {merged}"
    )
    ...
    try:
        for target in targets:
            ...
            if not source.is_dir():
                raise RuntimeError(
                    "Cannot merge missing standalone "
                    f"application folder: {source}"
                )

            info(
                f"Merging {target.directory} "
                f"into {MERGED_APPLICATION_DIRECTORY}"
            )

            _merge_application_tree(
                source,
                staging,
            )

        info(
            "Verifying all executables and required DLLs in the merged folder"
        )
    ...
    if main_target is not None:
        info(
            "Setting the merged application's Windows GPU preference"
        )
    ...
    info(
        f"Merged application folder complete: {merged}"
    )
...
def _is_full_application_build(
    ...
    """Return whether every configured application target is selected."""
    return (
...
def build(
    ...
    """Build the requested standalone application distributions."""
    ...
    for target in targets:
        if not target.icon.is_file():
            raise RuntimeError(
                f"Required application icon is missing: {target.icon}"
            )
    ...
    release = (
            DIST_ROOT
            / f"AIBrain_{release_timestamp}"
    )

    if release.exists():
        raise RuntimeError(
            f"Refusing to overwrite existing distribution: {release}"
        )
    ...
    info(
        f"Distribution directory: {release}"
    )

    info(
        "Checking required Visual C++ runtime DLLs"
    )
    ...
    try:
        for index, target in enumerate(
            ...
            numpy_runtime = stage_numpy_runtime(
                build_root
            )
            ...
            info(
                "Verifying executable and required DLLs: "
                f"{target.executable}"
            )
            ...
            if target.executable == "ai_brain.exe":
                info(
                    "Setting the application's Windows GPU preference"
                )
            ...
            info(
                f"Completed {target.executable} in "
                f"{time.monotonic() - target_started:.1f}s"
            )

        if _is_full_application_build(
                targets
        ):
            section(
...
def main() -> int:
    """Build timestamped standalone AIBrain desktop applications."""
    ...
    parser.add_argument(
        "--timestamp",
        help=(
            "Override the YYYYMMDD_HHMMSS distribution "
            "suffix for reproducible builds"
        ),
    )
    ...
    info(
        f"Log file: {runtime_log}"
    )

    info(
        "Native Nuitka progress bars, build stages, "
        "warnings, and results appear below"
    )
...
if __name__ == "__main__":
    try:
        ...
        report_exception(
            "AIBrain distribution build failed",
            exc,
        )
